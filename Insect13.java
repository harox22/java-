//加入constructor chain;不用重复写weight=initWeight,X,Y和population++部分代码

public class Insect13{
	private int X;
	private int Y;
	...
	/* 这部分可以用constructor chain改写下；
	//constructor1 ,constructor reloading~~~
	public Insect13(double initWeight){  //所以有两个相同名字的Insect11 constructor也可以
		X=Default_X;
		Y=Default_Y;
		Weight=initWeight;
		population++;
	...
	*/
	...
	//constructor chain:
	public Insect13(int initWeight){    //这里将initWeight设置为int了
		this(initWeight,Default_X,Default_Y);  //population不需要在这里写也会递增，为什么？
	}

	//constructor2
	public Insect13(int initX,int initY,double initWeight){
		X=initX;
		Y=initY;
		Weight=initWeight;	
